...
traverse(ast, {
    ForStatement(path)
    {
        if (path.node.init != null || path.node.update != null || path.node.test == null || !types.isIdentifier(path.node.test.left) || 
            !types.isMemberExpression(path.node.test.right) || path.node.body == null || path.node.body.body.length > 1 || 
            !types.isSwitchStatement(path.node.body.body[0]))
            return
        // || path.test.left.name != stateHolder
        ...
        let stateHolder = prev.node.declarations[0]
        if (!types.isMemberExpression(stateHolder.init) || !types.isIdentifier(stateHolder.id))
            return
            
        stateHolderName = stateHolder.id.name
        let initialState = generate(stateHolder.init).code
        initialState = initialState.substring(initialState.indexOf('['))        
        
        end = generate(path.node.test.right).code
        end = end.substring(end.indexOf('['))        
        path.node.test.right = types.numericLiteral(endIndex) // replace ending with endIndex
        
        for (let c of path.node.body.body[0].cases) {
            let cons = c.consequent
            if (types.isBreakStatement(cons[cons.length - 1]) && types.isExpressionStatement(cons[cons.length - 2])) {
                let assignment = cons[cons.length - 2].expression
                if (!types.isAssignmentExpression(assignment) || !types.isIdentifier(assignment.left) || assignment.left.name != stateHolderName)
                    return
                    
                let index = cases.length
                let caseCode = generate(c.test).code
                caseCode = caseCode.substring(caseCode.indexOf('['))
                cases[index] = caseCode
                c.test = types.numericLiteral(index)
                
                if (eval(`flowArray${initialState} == flowArray${caseCode}`))
                    stateHolder.init = types.numericLiteral(index)
            }
        }
    }
})
...
traverse(ast, {
    ...
    {
        let node = path.node
        if (!types.isIdentifier(node.left) || node.left.name != stateHolderName)
            return
        ...
        if (types.isMemberExpression(node.right)) {
            ...
            for (let i = 1; i < cases.length; i++) {
                ...
                if (eval(`flowArray${end} == flowArray${s}`))
                    path.node.right = types.numericLiteral(endIndex)
                else if (eval(`flowArray${c} == flowArray${s}`))
                    path.node.right = types.numericLiteral(i)
            }
        }
            ...
            for (let i = 1; i < cases.length; i++) {
                ...
                if (eval(`flowArray${end} == flowArray${cons}`))
                    path.node.right.consequent = types.numericLiteral(endIndex)
                else if (eval(`flowArray${end} == flowArray${alt}`))
                    path.node.right.alternate = types.numericLiteral(endIndex)
                else if (eval(`flowArray${c} == flowArray${cons}`))
                    path.node.right.consequent = types.numericLiteral(i)
                else if (eval(`flowArray${c} == flowArray${alt}`))
                    path.node.right.alternate = types.numericLiteral(i)
            }
        }
    }
})

...
traverse(ast, {
    ReturnStatement(path) {
        ...
        if (types.isObjectExpression(path.node.argument) && path.node.argument.properties.length == 1 &&
            types.isCallExpression(path.node.argument.properties[0].value) && types.isFunctionExpression(path.node.argument.properties[0].value.callee) &&
            path.node.argument.properties[0].value.arguments.length === 1 && types.isStringLiteral(path.node.argument.properties[0].value.arguments[0])
        ) {
            ...
            stringFunction = path.node.argument.properties[0].key.name
            stringArrayInitString = path.node.argument.properties[0].value.arguments[0].value
            //stringPath.remove()
        }
    },
...
traverse(ast, {
    FunctionDeclaration(path) {
        ...
        if (possibleStringArrayFunctions.includes(path.node.id.name) && path.node.body.body.length === 1 &&
            ...
            stringArrayString = decodeURI(stringArrayStringTemp)
            ...
            decodedStrings = decodedStrings.split(stringArraySeparator)
        }
    }
})
...
//  Once evals are unmasked, run string deconcealing (for ticket they use global variables instead of integers as arguments)
